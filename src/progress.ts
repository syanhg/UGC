...
type State = 'waiting' | 'running' | 'done' | 'failed';

interface Job {
  label: string;
  state: State;
  startedAt?: number;
  /** Frozen on completion, so a finished row keeps reporting its real time. */
  endedAt?: number;
  detail?: string;
}
...
export class Progress {
  ...
  constructor(labels: string[], estimateMs: number) {
  ...
  began(index: number): void {
    this.jobs[index].state = 'running';
    this.jobs[index].startedAt = Date.now();
    if (!this.live) console.log(`[${index + 1}/${this.jobs.length}] ${this.jobs[index].label}`);
  }

  finished(index: number, detail: string): void {
    this.jobs[index].state = 'done';
    this.jobs[index].endedAt = Date.now();
    this.jobs[index].detail = detail;
    if (!this.live) {
      console.log(
        `[${index + 1}/${this.jobs.length}] done in ` +
          `${Math.round(this.elapsed(this.jobs[index]) / 1000)}s → ${detail}`,
      );
    }
  }

  failed(index: number, detail: string): void {
    this.jobs[index].state = 'failed';
    this.jobs[index].endedAt = Date.now();
    this.jobs[index].detail = detail;
    if (!this.live) console.log(`[${index + 1}/${this.jobs.length}] FAILED`);
  }
  ...
  private elapsed(job: Job): number {
    if (!job.startedAt) return 0;
    return (job.endedAt ?? Date.now()) - job.startedAt;
  }

  private bar(job: Job): string {
    const ratio =
    ...
    const colour =
      job.state === 'failed' ? RED : job.state === 'done' ? GREEN : '';

    return `${colour}${'█'.repeat(filled)}${DIM}${'░'.repeat(WIDTH - filled)}${RESET}`;
  }

  private line(job: Job, index: number): string {
 ...
 */
export function estimateRenderMs(model: string, duration: number): number {
