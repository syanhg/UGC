 ...
 */
export async function fetchWithRetry(url, { attempts = 8, signal, headers, } = {}) {
    let lastError;
    for (let attempt = 1; attempt <= attempts; attempt++) {
        try {
            ...
            if (signal?.aborted)
                throw err;
            if (attempt < attempts) {
                await new Promise((r) => setTimeout(r, Math.min(2_000 * 2 ** (attempt - 1), 30_000)));
            }
        }
    }
